...
\begin{titlepage}
    ...
    \vfill
    {\large \textbf{Année universitaire :} 2025--2027\par}
...
\textbf{Explication du diagramme :} ce diagramme présente les principales
interactions entre les acteurs et l'application. Le visiteur dispose seulement
des actions d'inscription et de connexion. Après authentification, il devient
utilisateur et peut accéder aux fonctionnalités musicales : recherche, écoute,
favoris, playlists et partage. L'administrateur hérite des usages généraux de
l'application, mais possède en plus l'accès à la gestion des utilisateurs.

Cette représentation permet de visualiser rapidement le périmètre fonctionnel
du projet et de distinguer les fonctionnalités publiques, les fonctionnalités
réservées aux utilisateurs connectés et les fonctionnalités réservées aux
administrateurs.
...
\textbf{Explication du diagramme :} le diagramme de classes décrit les entités
centrales utilisées par le backend. La classe \code{UserAccount} représente un
compte utilisateur et contient les informations nécessaires à l'identification,
au profil et au rôle. La classe \code{AuthToken} est liée à un utilisateur et
sert à conserver les jetons de session utilisés pour authentifier les requêtes.

La classe \code{FavoriteTrack} permet de sauvegarder un titre dans la
bibliothèque personnelle d'un utilisateur. Elle garde les informations
essentielles du morceau afin de pouvoir l'afficher rapidement dans les favoris.
La classe \code{TrackDto} représente un titre normalisé venant d'une API
musicale externe. Le lien en pointillés montre que le favori référence un titre
externe, mais que ce titre n'est pas une entité persistée de la même manière
qu'un utilisateur ou un jeton.
...
\textbf{Explication du diagramme :} ce diagramme montre le chemin suivi par les
données entre l'utilisateur et les différentes couches du système. Le navigateur
communique avec le frontend Angular, qui se charge de l'interface, des
formulaires et des interactions utilisateur. En développement local, Angular
redirige les requêtes commençant par \path{/api} vers le backend Spring Boot
grâce au proxy.

Le backend joue le rôle de point central. Il applique les règles métier,
contrôle la sécurité, interroge MySQL pour les données internes et contacte les
API musicales pour récupérer les titres. Cette organisation évite d'exposer
directement la base de données ou les clés éventuelles des services externes
côté client.

Les échanges entre les couches suivent un format JSON. Les controllers REST
reçoivent les requêtes, les services appliquent les règles métier, les
repositories accèdent aux données persistées et les mappers construisent les
DTO retournés au frontend. Cette séparation rend le code plus lisible et limite
les dépendances entre les couches.
